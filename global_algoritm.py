...
def update ():
    ...
    WHERE lerning_profession.type = 'ОТ' AND lerning_profession.number = 1  AND training_report_OT.curse_num = 1 AND strftime('%Y', date('now')) - strftime('%Y', training_report_OT.date) > (SELECT curse.year FROM curse WHERE curse.number = 1 AND curse.type = 'ОТ')
    ...
    sqlite_select = """SELECT employee.ID, employee.profession, lerning_division.month
        ...
        JOIN lerning_division ON employee.division = lerning_division.division
            JOIN lerning_profession ON employee.profession = lerning_profession.ID
            JOIN training_report_OT ON employee.ID = training_report_OT.ID_employee 
            WHERE lerning_profession.type = 'ОТ' AND lerning_profession.number = 2  AND training_report_OT.curse_num = 2 AND strftime('%Y', date('now')) - strftime('%Y', training_report_OT.date) > (SELECT curse.year FROM curse WHERE curse.number = 2 AND curse.type = 'ОТ')
            GROUP BY ID_employee
            """
    cursor.execute(sqlite_select)
    ...
    for row in cursor.fetchall():
        ...
        cursorObj2 = con2.cursor()
            # if row[3] == "Машинист мельниц" or row[3] == "Сепараторщик" or row[3] == "Машинист конвейера" or row[3] == "Машинист насосных установок" or row[3] == "Машинист установок по разрушению "
        if row[1] == 4 or row[1] == 6 or row[1] == 7 or row[1] == 1 or row[1] == 8 or row[1] == 9:
            albums = (row[0], 'Рабочие', 2, row[1], row[2])
        elif row[1] == 0:
            albums = (row[0], 'Специалист', 2, row[1], row[2])
        elif row[1] == 2 or row[1] == 3 or row[1] == 5:
            albums = (row[0], 'Руководители', 2, row[1], row[2])
        else:
    ...
    sqlite_select = """SELECT employee.ID, employee.profession, lerning_division.month
            ...
            WHERE training_report_OT.ID_employee IS NULL AND lerning_profession.type = 'ОТ' AND lerning_profession.number = 2
