...
def handle_document_number(message, index, curse_num, plan_type):
    ...
    if plane_details:
        ...
        ask_training(message.chat.id, fetch_training_plans(month=int(start_date.split('-')[1]), plan_type=plan_type),
                     index + 1, plan_type)
...
GROUP BY
    plane_OT.number, plane_OT.end_date, employee.division"""
        ...
        markup = telebot.types.InlineKeyboardMarkup()
        markup.add(
            telebot.types.InlineKeyboardButton("Неявка", callback_data=f"not_show_{index}_{curse_num}_{plan_type}"),
            telebot.types.InlineKeyboardButton("Не сдано",
                                               callback_data=f"not_submitted_{index}_{curse_num}_{plan_type}"))
        ...
        if not plans:
            bot.send_message(call.message.chat.id,
                             f"Не было запланировано обучений в этом месяце для типа {plan_type_name}.")
            return

        ask_training(call.message.chat.id, plans, 0, plan_type)

    elif (call.data.startswith('not_')):
        ...
        bot.send_message(call.message.chat.id, "Выберите месяц для заполнения информации о прошедшем обучении:",
                         reply_markup=markup)
